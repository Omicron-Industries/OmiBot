            ...
            match ctx.consume_arg() {
                Some(perm_str) => {
                    if let Some(perm) = Permission::from_name(&perm_str) {
                        if let Err(e) = add_admin_permission(
                            ctx.get_guild_id(),
                            uid,
                            perm.value(),
                            &ctx.state.db_pool,
                        ...
                        .await
                        {
                            send_reply_ping_text(
                                ctx,
                                format!("Failed to add admin permission: {}", e).as_str(),
                            )
                            .await;
                        } else {
                            send_reply_ping_text(ctx, "Admin permission added successfully.").await;
                        }
                    } else {
